...
public partial class RustBridge
{
    ...
    private static partial IntPtr NewEntry(string entryName, string arg, string entryDateTime);
    ...
    public static WorkHours CreateWorkHours(string start, string end)
    {
        ...
        FreeString(ptr);
        WorkHours hours = JsonSerializer.Deserialize<WorkHours>(json);
        return hours;
    }
    ...
    public static WorkTask CreateTask(string taskName, string arg, string taskTime)
    {
        ...
        string json = Marshal.PtrToStringUTF8(ptr)!;
        FreeString(ptr);
        WorkTask task = JsonSerializer.Deserialize<WorkTask>(json);
        return task;
    }

    public static Entry CreateEntry(string entryName, string arg, string entryDateTime)
    {
        IntPtr ptr = NewEntry(entryName, arg, entryDateTime);
        string json = Marshal.PtrToStringUTF8(ptr)!;
        FreeString(ptr);
